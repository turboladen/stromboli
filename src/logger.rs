...
impl Logger {
    ...
    pub fn log_msg<T: Display>(&self, msg: T) {
        log_msg(self.subject_name, msg);
    }

    pub fn log_sub_msg<T: AsRef<str>, U: AsRef<str>>(&self, sub_name: T, msg: U) {
        log_sub_msg(self.subject_name, sub_name.as_ref(), msg.as_ref());
    }

    pub fn log_sub_footer<T: AsRef<str>>(&self, sub_name: T) {
        log_sub_footer(self.subject_name.bold(), sub_name.as_ref().bold());
    }

    pub fn log_footer(&self) {
        log_footer(self.subject_name.bold());
    }
